...
// Optimized PHP login page paths (removed wp-admin and phpmyadmin)
...
func findLoginPagesOptimized(ip string, port int) []LoginPageResult {
	...
	var allResults []LoginPageResult
	...
	for _, path := range loginPaths {
		if result := checkLoginPage(baseURL, path); result != nil {
			...
			if existingResult, exists := finalURLMap[result.FinalURL]; exists {
				...
				// New final URL, add to map and results
...
func main() {
	...
	filename := fmt.Sprintf("php_optimized_results_%s.txt", timestamp)
	...
	header := fmt.Sprintf("PHP Login Page Scanner (Optimized) Results - %s\nScanning %d random IPs with %d concurrent workers\nLooking for PHP applications with login pages\nOptimizations: Removed wp-admin/phpmyadmin paths, deduplicates redirect results\nSupported: WordPress, Laravel, Drupal, Joomla, CodeIgniter, CakePHP, Symfony, Magento, PrestaShop, OpenCart\n%s\n\n",
		time.Now().Format("2006-01-02 15:04:05"), totalScans, maxConcurrency, strings.Repeat("=", 80))
	file.WriteString(header)

	fmt.Printf("🐘 PHP LOGIN PAGE SCANNER (OPTIMIZED) STARTING...\nResults will be saved to: %s\n\n", filename)
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				statsMu.Lock()
				fmt.Printf("🐘 PHP Optimized Progress: %d IPs scanned, %d PHP sites with results found\n", scannedCount, foundCount)
				statsMu.Unlock()
			default:
