...
// Common ASPX login page paths to check
...
// findLoginPagesOptimized scans for login pages with deduplication
func findLoginPagesOptimized(ip string, port int) []LoginPageResult {
	// Determine protocol
	protocol := "http"
	if httpsPort, exists := httpsPort[port]; exists && httpsPort {
		protocol = "https"
	...
	// Map to track final URLs to avoid duplicates
	finalURLMap := make(map[string]*LoginPageResult)

	// Check each potential login path
	for _, path := range loginPaths {
		if result := checkLoginPage(baseURL, path); result != nil {
			// Check if we've already seen this final URL
			if existingResult, exists := finalURLMap[result.FinalURL]; exists {
				// If this is a redirect and we have a better original URL, update it
				if result.Redirected && (len(result.URL) < len(existingResult.URL) || result.URL == baseURL+"/") {
					existingResult.URL = result.URL
				}
			} else {
				// New final URL, add to map
				finalURLMap[result.FinalURL] = result
			}
		}
	...
	var deduplicatedResults []LoginPageResult
	for _, result := range finalURLMap {
	...
	for ip := range jobs {
		...
		for _, port := range openPorts {
			if checkASPX(ip, port) {
				...
				// Found ASPX site, now look for login pages with optimization
				loginPages := findLoginPagesOptimized(ip, port)
				...
				if len(loginPages) > 0 {
					...
					for _, loginPage := range loginPages {
						...
						if loginPage.Redirected {
							msg = fmt.Sprintf("[ASPX LOGIN REDIRECT] %s -> %s | %s | Indicators: %d | Status: %d",
								loginPage.URL, loginPage.FinalURL, loginPage.Title, len(loginPage.LoginIndicators), loginPage.StatusCode)
						} else {
							msg = fmt.Sprintf("[ASPX LOGIN FOUND] %s | %s | Indicators: %d | Status: %d",
...
func main() {
	...
	filename := fmt.Sprintf("aspx_optimized_results_%s.txt", timestamp)
	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				statsMu.Lock()
				fmt.Printf("🔷 ASPX Optimized Progress: %d IPs scanned, %d ASPX sites with results found\n", scannedCount, foundCount)
				statsMu.Unlock()
			default:
